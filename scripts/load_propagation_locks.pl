...
Trace("Recreating fid propagation_locks table.") if T(2);
$dbf->drop_table( tbl => "fid_propagation_locks" );
$dbf->create_table( tbl  => "fid_propagation_locks",
		    flds => "fid varchar(32)"
		  );
...
foreach $org (@orgs)
{
	...
	foreach $type (@types)
	{
	    my $file = "$FIG_Config::organisms/$org/Features/$type/propagation_locks";
	    if (open(IN,"<$file"))
	    {
		my %set;
		...
		close(OUT);
		$dbf->load_table( tbl => "fid_propagation_locks", 
		                  file => $tmpF
				);
	    }
	}
...
$dbf->create_index( idx  => "fid_propagation_locks_ix",
		    tbl  => "fid_propagation_locks",
		    type => "btree",
		    flds => "fid" );
